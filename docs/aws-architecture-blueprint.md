...
This repository is not currently a public production deployment or hosted scanner. Production AWS deployment automation remains future work; there is no Terraform/CDK pipeline, no public endpoint, no production RDS migration plan, and no managed multi-user storage in v0.8.0-beta.
