...
run_check() {
  ...
  if output=$(git grep -n -I -E "$pattern" -- . "${base_excludes[@]}" 2>/dev/null | grep -Eiv "$allowlist" || true); then
    if [[ -n "$output" ]]; then
      printf '\n%s\n' "$label"
      printf '%s\n' "$output"
      status=1
...
run_check "Private key header matches:" '-----BEGIN (RSA |DSA |EC |OPENSSH |PGP |ENCRYPTED )?PRIVATE KEY-----'
